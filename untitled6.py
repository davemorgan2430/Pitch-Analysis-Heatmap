...
# Section: League Average Heatmap for Arm Slot
st.header("League Average Heatmap for Arm Slot")

# User input: Select pitcher
league_player_name = st.text_input("Enter a player's name to compare league averages:", "")

if league_player_name:
    # Filter data for selected player
    player_data = df[df['player_name'] == league_player_name]
    
    if player_data.empty:
        st.warning("Player not found. Please check the name or try another.")
    else:
        # Calculate player's average arm angle
        player_arm_angle = player_data['arm_angle'].mean()

        # Filter league data for similar arm angle
        arm_angle_range = 5  # Adjustable range
        league_filtered = df[
            (df['arm_angle'] >= player_arm_angle - arm_angle_range) &
            (df['arm_angle'] <= player_arm_angle + arm_angle_range)
        ]

        # Check if there are enough league data points
        if league_filtered.empty:
            st.warning("No league data found for pitchers in the specified arm slot range.")
        else:
            # Create a KDE heatmap for league average movement
            plt.figure(figsize=(10, 8))
            sns.kdeplot(
                ...
                y='iVB',
                fill=True,
                cmap='coolwarm',  # Change to a preferred colormap
                thresh=0.05,      # Adjust threshold for contours
                levels=10,        # Number of contour levels
                cbar=True,        # Add color bar
            )

            # Highlight the selected pitcher's average HB and iVB
            average_hb = player_data['HB'].mean()
            average_ivb = player_data['iVB'].mean()
            ...
            plt.scatter(
                ...
                color='orange',
                s=100,
                label=f"{league_player_name} (Avg HB: {average_hb:.2f}, Avg iVB: {average_ivb:.2f})"
            )
            ...
            # Set labels and title
            plt.title(
                f"League Average Heatmap | Arm Slot: {player_arm_angle:.1f}°",
                fontsize=16
            )
            plt.xlabel("Horizontal Break (HB)", fontsize=14)
            plt.ylabel("Induced Vertical Break (iVB)", fontsize=14)
